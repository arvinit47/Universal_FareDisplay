...
// ---------- Serial auto-reconnect & safe listeners (paste into main.js) ----------

// Safe close: remove listeners, unpipe parser, close port and clear buffer
async function safeCloseExistingPort() {
  if (!serialPort) return;
  try {
    // remove listeners and unpipe parser if present
    try { serialPort.removeAllListeners(); } catch (e) {}
    try { if (parser && typeof serialPort.unpipe === "function") serialPort.unpipe(parser); } catch (e) {}
    try { parser?.removeAllListeners?.(); } catch (e) {}
    ...
    console.warn("safeCloseExistingPort warning:", err && (err.message || err));
  } finally {
    serialPort = null;
    parser = null;
    // important: clear buffer to avoid re-processing stale frames
    serialPortData = "";
  }
}

// Named handlers (attach / detach these to avoid duplicates)
function onSerialData(raw) {
  try {
    const ascii = raw.toString();
    console.log("Raw ASCII:", ascii);

    // special clear control
    if (ascii.slice(1, 3) === "13") {
      mainWindow?.webContents.send("clear-screen", "clear");
      serialPortData = "";
      return;
    ...
    try {
      ...
      console.error("processData error:", procErr && (procErr.message || procErr));
      mainWindow?.webContents.send("serial-error", procErr && (procErr.message || String(procErr)));
      serialPortData = ""; // drop junk
      return;
    }
    ...
    if (parsed && Object.keys(parsed).length > 0) {
      ...
      // drop processed frames up to last '^'
      const lastIdx = serialPortData.lastIndexOf("^");
      serialPortData = lastIdx >= 0 ? serialPortData.slice(lastIdx + 1) : "";
    }
  } catch (err) {
    console.error("Unhandled error in onSerialData:", err && (err.message || String(err)));
    mainWindow?.webContents.send("serial-error", err && (err.message || String(err)));
  }
}

function onSerialError(err) {
  console.error("Serial port error:", err && (err.message || String(err)));
  mainWindow?.webContents.send("serial-error", err && (err.message || String(err)));
  scheduleReconnect(); // schedule reconnect with the current settings
}

function onSerialClose() {
  console.warn("Serial port closed.");
  mainWindow?.webContents.send("serial-error", "port-closed");
  // clear buffer on close to avoid reprocessing stale data on reopen
  serialPortData = "";
  scheduleReconnect();
}

// attach listeners safely (removes prior listeners first)
function setupSerialListeners() {
  ...
  // also clear any anonymous listeners defensively
  try {
  ...
  // reset buffer when (re)attaching
  serialPortData = "";
  ...
  console.log("Serial listeners attached.");
}

// open the serial port (safe) and setup listeners on success
function openSerialPort(portName, baudRate) {
  // ensure any previous port is closed first
  (async () => {
    await safeCloseExistingPort();
    ...
    try {
      ...
      serialPort.open((err) => {
        if (err) {
          console.error(`Error opening ${portName}:`, err && (err.message || String(err)));
          mainWindow?.webContents.send("serial-error", err && (err.message || String(err)));
          scheduleReconnect();
          return;
        }
        console.log(`Serial Port ${portName} is Open`);
        // reset buffer on successful open to avoid stale frames
        serialPortData = "";
        setupSerialListeners();
      });
    } catch (err) {
      console.error("Serial port initialization error:", err && (err.message || String(err)));
      mainWindow?.webContents.send("serial-error", err && (err.message || String(err)));
      scheduleReconnect();
    }
  })();
...
// schedule reconnect (idempotent)
function scheduleReconnect() {
  if (reconnectTimer) return; // already scheduled
  reconnectTimer = setTimeout(() => {
    reconnectTimer = null;
    const portName = settingsData?.port || "/dev/serial0";
    const baudRate = Number(settingsData?.baudRate) || 9600;
    console.log("Retrying to open serial port:", portName);
    openSerialPort(portName, baudRate);
  }, reconnectDelay);
}

// initializeSerialPort: call this where you currently call it (e.g. in createWindow)
function initializeSerialPort() {
  const portName = settingsData ? `${settingsData.port}` : "/dev/serial0";
  const baudRate = settingsData ? Number(settingsData.baudRate) : 9600;
  openSerialPort(portName, baudRate);
